...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 21 Jun 2021 pada 07.29
...
-- Prosedur
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SPstok0` ()  NO SQL
SELECT tb_barang.*,tb_kategori.kategori,tb_merk.merk from tb_barang JOIN tb_merk ON tb_barang.kode_merk = tb_merk.kode_merk JOIN tb_kategori ON tb_barang.kode_kategori = tb_kategori.kode_kategori WHERE stok = 0$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SPtampilPembelianTerbanyak` ()  NO SQL
Select a.total,a.tgl_transaksi,a.status_transaksi,b.nama from tb_transaksi a JOIN tb_customer b on a.kode_customer = b.kode_customer ORDER BY total DESC LIMIT 2$$
...
(21, 'BRG004', 'TRX004', 'CUS001', 1, 9998000);
...
(6, 'TRX004', 'CUS001', 9998000, '', 'Transfer', '2021-06-21 00:33:19', 'Menunggu Konfirmasi');
